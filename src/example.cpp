...
int main()
{
    ...
    buildFoo2 builder2;
    std::cout << "Instantiate Foo2 with integer and double data types" << std::endl;
    IFoo * foo2 = for_id<first_list, second_list, IFoo>(builder2, input, input.id1, input.id2);
    foo2->printData();
    foo2->printMoreData();
    foo2->printEvenMoreData();
    delete foo2;

    /*
    buildFoo3 builder3;
    std::cout << "Instantiate Foo3 with integer, float and double data types" << std::endl;
    IFoo * foo3 = for_id<first_list, second_list, third_list, IFoo>(builder3, input, input.id1, input.id2, input.id3);
    foo3->printData();
    foo3->printMoreData();
    foo3->printEvenMoreData();
    delete foo3;
    */
